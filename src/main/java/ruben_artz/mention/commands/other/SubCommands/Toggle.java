package ruben_artz.mention.commands.other.SubCommands;

import org.bukkit.Bukkit;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;
import ruben_artz.mention.DeluxeMentions;
import ruben_artz.mention.commands.other.SubCommand;
            ...
            UtilityFunctions.runTask(() -> {
                if (sender instanceof Player) {
                    ...
                    if (plugin.IgnoreMention.contains(player.getUniqueId())) {
                        ...
                        UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfig().getString("MENTION.ACTIVATED_SOUND")), player);
                        ...
                        UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfig().getString("MENTION.DISABLED_SOUND")), player);
        ...
        if (args.length >= 2) {
            ...
            UtilityFunctions.runTask(() -> {
                if (plugin.IgnoreMention.contains(target.getUniqueId())) {
                    ...
                    Launcher.getInstance().getCache().set(target.getUniqueId(), "MENTION", true);
                    UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfig().getString("MENTION.ACTIVATED_SOUND")), target);
                    ...
                    Launcher.getInstance().getCache().set(target.getUniqueId(), "MENTION", false);
                    UtilityFunctions.executeSound(Objects.requireNonNull(plugin.getConfig().getString("MENTION.DISABLED_SOUND")), target);
